...
5. `SessionManager.applyCandidateCookies` injects the candidate into YouTubeKit in memory. `AccountService.fetchAccountInfo()` then asks YouTube to validate it; when YouTubeKit's account parser defaults to disconnected because `loggedOut` is absent, a raw account-menu probe distinguishes an explicit rejection from a changed authenticated response shape.
