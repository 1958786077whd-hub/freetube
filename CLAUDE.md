...
2. Login starts at Google `ServiceLogin` with `service=youtube` and a YouTube sign-in continuation. `LoginCoordinator` watches navigation; when the URL transitions to `youtube.com`, it polls the persistent web view's `httpCookieStore` for up to about 20 seconds and filters for `.youtube.com` / `.google.com` domains. Switching accounts clears only YouTube cookies and returns to Google AccountChooser.
...
4. There is no fixed all-or-nothing cookie set. Seeing any modern authentication candidate (`SAPISID`, `__Secure-1PAPISID`, `__Secure-3PAPISID`, `__Secure-1PSID`, `__Secure-3PSID`, `SID`, or `APISID`) that is scoped so a browser can send it to `www.youtube.com` is enough to construct the complete candidate header. Google-only cookies remain in the header but cannot prematurely start verification.
5. `SessionManager.applyCandidateCookies` injects the candidate into YouTubeKit in memory. `AccountService.fetchAccountInfo()` sends that exact header to a fresh YouTube web request and reads the server-generated `LOGGED_IN` result. This is the primary truth source because the pinned YouTubeKit InnerTube client can be rejected independently of a valid browser session. AccountInfos remains a best-effort metadata request; raw account-menu probing is used only when the web response is indeterminate.
